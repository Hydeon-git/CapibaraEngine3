...
void FileSystem::LoadFile(std::string& path)
{
	...
	for (s = texExtension.begin(); s != end; ++s)
	{
		if (*s == extension)
		{
			CB_PROFILING_FUNCTION("Loading Texture");
...
void FileSystem::LoadFiles()
{
	...
	while (!dirsStack.empty())
	{
		...
		for (std::vector<std::string>::iterator it = files.begin(); it != files.end(); ++it)
		{
			if ((*it).find(".cbmesh") != std::string::npos)
			{
...
ResourceType FileSystem::CheckExtension(std::string& path)
{
	...
	if (extension.data() == std::string(".h")) return ResourceType::SCRIPT;
	...
	for (s = texExtension.begin(); s != end; ++s)
	{
		if (*s == extension)
		{
			CB_PROFILING_FUNCTION("Importing Texture");
