...
#ifndef GAME_VERSION
	GameObject* light_test = new GameObject(baseGo);
...
//bool ModuleScripting::IsInvoking(std::function<void()> void_no_params_function)
//{
...
void ModuleScripting::CreateJsonScript(GameObject* obj, JSONArraypack* to_save)
{
	if (obj->HasChildren()) {
		std::vector<GameObject*>::iterator item = obj->children.begin();
		for (; item != obj->children.end(); ++item) {
