...
public:
	...
	void OnPreCull(CameraComponent* camera) const;
	void OnPreRender(CameraComponent* camera) const;
	void OnPostRender(CameraComponent* camera) const;
	/*---------Scripts Calls-----------*/
...
public:
	...
	Octree octree;
	std::stack<ReturnZ*> returnActions;
	std::stack<ReturnZ*> fordwardActions;

	bool in_cntrl_Z = false;

	std::vector<CameraComponent*> gameCameras;
	bool enableInstancies = true;
	ResourcePrefab* prefabOpened = nullptr;
	bool ignoreCtrlZ = false;

	ComponentScript* actualScriptLoading = nullptr;
...
private:
	...
	std::vector< std::tuple<GameObject*, GameObject*, bool>> toReparentGo;

	std::stack<ReturnZ*> saveReturnActions;
	std::stack<ReturnZ*> saveFordwardActions;

	std::vector<std::pair<u64, GameObject**>> toAdd;
